    ...
    public class EmployeeController : ApiController
    {
        ...
        public IHttpActionResult Save(EmployeeModel model)
        {
            ...
            else
            {
                ...
                foreach (var item in dbEmployee.UserRecipients.Where(a => !obj.UserRecipients.Any(b => b.ID == a.ID)).ToList())
                {
                    item.State = State.Deleted;
                }
                foreach (var item in dbEmployee.UserDoners.Where(a => !obj.UserDoners.Any(b => b.ID == a.ID)).ToList())
                {
                    item.State = State.Deleted;
                }
                foreach (var item in obj.UserDoners)
                {
                    if (item.ID == null)
                    {
                        item.State = State.Added;
                        dbEmployee.UserDoners.Add(item);
                    }

                }
                foreach (var item in obj.UserRecipients)
                {
                    if (item.ID == null) { 
                        item.State = State.Added;
                        dbEmployee.UserRecipients.Add(item);
                    }
                }
