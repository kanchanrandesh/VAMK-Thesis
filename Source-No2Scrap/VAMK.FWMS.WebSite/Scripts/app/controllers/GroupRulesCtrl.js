angular.module('MetronicApp').controller('GroupRulesCtrl', function ($rootScope, $scope, $http, $timeout, $document, $uibModal, $stateParams, $state, groupService, notificationMsgService) {
    ...
    (function () {
        loadGroupRules().then(fillMultiSelect);
    }());

    function loadGroupRules() {
        var defer = $.Deferred();
        groupService.getForGroupRules($stateParams.id).then(function (res) {
            $scope.group = res;
    ...
    $scope.save = function (obj, frm) {
        if (frm.$valid) {
            groupService.saveRules(obj).then(function (res) {
                if (res.status == true) {
