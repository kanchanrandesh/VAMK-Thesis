...
namespace VAMK.FWMS.DataObjects.Mappers
{
    internal class EmployeeMapper : EntityTypeConfiguration<Employee>
    {
        public EmployeeMapper()
        {
            ...
            HasMany(t => t.EmployeeDoners).WithOptional(l => l.Employee).HasForeignKey(t => t.EmployeeID).WillCascadeOnDelete(true);
            HasMany(t => t.EmployeeRecipients).WithOptional(l => l.Employee).HasForeignKey(t => t.EmployeeID).WillCascadeOnDelete(true);
            #endregion
        }
    }
}
